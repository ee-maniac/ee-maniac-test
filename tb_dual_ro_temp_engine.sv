...
module tb_dual_ro_temp_engine;
    ...
    real actual_duration_ns;
        ...
        for (i = 0; i < NUM_TESTS; i = i + 1) begin
            ...
            // -------------------------------------------------------------
            // Check that busy remains HIGH during the entire measurement
            // (from now until valid goes high)
            // -------------------------------------------------------------
            busy_dropped_early = 0;

            fork
                begin : busy_monitor
                    // Monitor every clock cycle until valid becomes 1
                    while (!valid) begin
                        @(posedge clk);
                        if (!busy) busy_dropped_early = 1;
                    end
                end
            join_none
            ...
            // Kill the monitor
            disable busy_monitor;

            // Check the result
            if (busy_dropped_early) begin
                $display("FAIL: busy went low BEFORE valid (test %0d)", i);
            end else begin
                $display("PASS: busy stayed high continuously until valid (test %0d)", i);
            end

            // Also check that busy is high on the valid edge (redundant but safe)
            if (!busy) begin
